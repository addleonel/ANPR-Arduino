...
class PlateModel(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name='userplate')
    license_plate = models.CharField(
        max_length=255, blank=False, null=False, unique=True)
    brand = models.CharField(max_length=255, blank=True)
    color = models.CharField(max_length=255, blank=True)
    status = models.CharField(
        max_length=255, blank=True, default='ADMITED', choices=OPTIONS)
    description = models.TextField(blank=True)
    slug = models.SlugField(null=True, unique=True, max_length=300)
    image = models.ImageField(
        upload_to='img/license_plates/', blank=True, null=True, max_length=300)
...
class PlateCapturedModel(models.Model):
    ...
    image_car = models.ImageField(
        upload_to='img/car_captured/', blank=True, null=True, max_length=300)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
